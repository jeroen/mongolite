...
Assume we want to tunnel through `dev.opencpu.org` which runs an SSH server on the standard port 22 with username `jeroen`. To initiate a from `localhost:9999` to `ds043942.mongolab.com:43942` via the tunnel server `dev.opencpu.org:22` open a terminal and run:
...
Some important flags:
 ...
 - `-i /some/path/id_rsa` to set a custom private key to connect

It is also possible to run this command directly from R:

```{r}
system("ssh -L 9999:ds043942.mongolab.com:43942 jeroen@dev.opencpu.org -fN -p22")
```

Now once tunnel has been established, we can connect to the the ssh client on the port we specified. In our example we run the ssh client on our local host, so we connect via:
